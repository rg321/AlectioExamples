...
def build_targets(pred_boxes, pred_conf, pred_cls, target, anchor_wh, nA, nC, nG, requestPrecision):
    ...
    anchor_wh = anchor_wh.cpu()

    if type(target) != list:
        target = target.data.numpy().tolist()
    # print(type(pred_boxes), type(pred_conf), type(pred_cls), type(target), type(anchor_wh), type(nA), type(nC), type(nG), type(requestPrecision))
    # print(len(pred_boxes), len(pred_conf), len(pred_cls), len(target), len(anchor_wh), type(nA), type(nC), type(nG), type(requestPrecision))
    # print(pred_boxes[0], pred_conf[0], pred_cls[0], target[0], anchor_wh[0], nA, nC, nG, requestPrecision)
    ...
    for b in range(nB):
        ...
        iou_anch = inter_area / (gw.cpu() * gh.cpu() + box2.prod(2) - inter_area + 1e-16)
        ...
        if nTb > 1:
            ...
            u = gi.cpu().float() * 0.4361538773074043 + gj.cpu().float() * 0.28012496588736746 + a.cpu().float() * 0.6627147212460307
        ...
        # Coordinates
        tx[b, a, gj, gi] = gx.cpu() - gi.cpu().float()
        ty[b, a, gj, gi] = gy.cpu() - gj.cpu().float()
        # Width and height
        tw[b, a, gj, gi] = torch.sqrt(gw.cpu() / anchor_wh[a, 0]) / 2
        th[b, a, gj, gi] = torch.sqrt(gh.cpu() / anchor_wh[a, 1]) / 2
        ...
        if requestPrecision:
            ...
            TP[b, i] = ((pconf > 0.99) & (iou_pred > 0.5) & (pcls.cpu() == tc.cpu())).type(torch.ByteTensor)
            FP[b, i] = ((pconf > 0.99) & (TP[b, i] == 0)).type(torch.ByteTensor)  # coordinates or class are wrong
            FN[b, i] = (pconf <= 0.99).type(torch.ByteTensor)  # confidence score is too low (set to zero)
