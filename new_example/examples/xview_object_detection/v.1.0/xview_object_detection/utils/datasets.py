...
class ListDatasetOld():  # for training
...
class ListDataset():  # for training
    def __init__(self, path, labeled, batch_size=1, img_size=608, targets_path=''):
        self.path = path
        self.labeled = labeled
        self.files = sorted(glob.glob('%s/*.tif' % path))
        print("TOTAL FILES {}".format(len(self.files)))
        self.files = [p for (k, p) in enumerate(self.files) if k in labeled]
        self.nF = len(self.files)  # number of image files
        self.nB = math.ceil(self.nF / batch_size)  # number of batches
        self.batch_size = batch_size
        assert self.nB >= 0, 'No images found in path %s' % path
        ...
        # load targets
        self.mat = scipy.io.loadmat(targets_path)
        self.mat['id'] = np.array(self.mat['id']).squeeze()
        self.class_weights = xview_class_weights(range(60)).numpy()
    ...
    def __iter__(self):
        self.count = -1
        # self.shuffled_vector = np.random.permutation(self.nF)  # shuffled vector
        self.shuffled_vector = np.random.choice(np.array(self.mat['image_numbers']).ravel(), self.nF,
                                                p=np.array(self.mat['image_weights']).ravel() / np.sum(np.array(self.mat['image_weights']).ravel()))
        return self
    ...
    def __next__(self):
        ...
        for index, files_index in enumerate(range(ia, ib)):
            # img_path = self.files[self.shuffled_vector[files_index]]  # BGR
            img_path = self.files[files_index]
            # img_path = '/Users/glennjocher/Downloads/DATA/xview/train_images/2294.bmp'
        ...
        # self.max_objects = 1800
        # filled_labels = np.zeros((len(labels_all), self.max_objects, 5))
        #
        # for k, label in enumerate(labels_all):
        #     filled_labels[k, :len(label), :] = label
        #
...
def random_affine(img, targets=None, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-3, 3),
    ...
    if targets is not None:
        ...
        if len(targets) > 0:
            n = np.array(targets).shape[0]
            points = np.array(targets)[:, 1:5].copy()
